...
namespace task_schedule {
	...
	namespace internal {
		...
		public:
			...
			explicit RetainedRefWrapper(utils::scoped_refptr<T> o) : ptr_(std::move(o)) {}
			T* get() const { return ptr_.get(); }

		private:
			utils::scoped_refptr<T> ptr_;
		};
		...
		std::enable_if_t<
			...
			assert(receiver->HasAtLeastOneRef() && "base::Bind{Once,Repeating}() refuses to create the first reference "
		...
		public:
			using Type = BindState<
				...
				utils::scoped_refptr<std::remove_pointer_t<DecayedReceiver>>,
				DecayedReceiver>,
	...
	template <typename T>
	struct IsWeakReceiver<utils::WeakPtr<T>> : std::true_type {};
