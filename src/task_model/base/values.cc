...
Value::Value(const std::vector<char>& in_blob)
    : data_(std::in_place_type_t<BlobStorage>(),
            in_blob.begin(),
            in_blob.end()) {}
...
Value::Value(const DictStorage& in_dict)
    : data_(std::in_place_type_t<DictStorage>()) {
...
Value::Value(span<const Value> in_list)
    : data_(std::in_place_type_t<ListStorage>()) {
...
double Value::AsDoubleInternal() const {
  return bit_cast<double>(std::get<DoubleStorage>(data_));
}

Value Value::Clone() const {
  return std::visit([](const auto& member) { return Value(member); }, data_);
}
...
const std::string* Value::FindStringKey(StringPiece key) const {
  const Value* result = FindKey(key);
  return result ? std::get_if<std::string>(&result->data_) : nullptr;
}
...
const Value::BlobStorage* Value::FindBlobKey(StringPiece key) const {
  const Value* result = FindKey(key);
  return result ? std::get_if<BlobStorage>(&result->data_) : nullptr;
}
