...
// in place.
	...
	class InternalHeapHandleStorage {
	public:
	...
	namespace intrusive_heap {
		...
		inline size_t ParentIndex(size_t i) {
			assert(0u != i);
			return (i - 1) / 2;
		}

		inline size_t LeftIndex(size_t i) {
			return 2 * i + 1;
		}
		...
		inline void CheckInvalidOrEqualTo(HeapHandle handle, size_t index) {
			if (handle.IsValid())
				assert(index == handle.index());
		}
	...
	void IntrusiveHeap<T, Compare, HeapHandleAccessor>::ClearHeapHandle(
		...
		assert(!GetHeapHandle(i).IsValid());
	}
	...
	void IntrusiveHeap<T, Compare, HeapHandleAccessor>::MoveHole(
		...
		assert(new_hole_pos != old_hole_pos);
		assert(new_hole_pos < size());
		assert(old_hole_pos <= size());
	...
	typename IntrusiveHeap<T, Compare, HeapHandleAccessor>::size_type
		...
		assert(hole_pos < n);
		assert(!GetHeapHandle(hole_pos).IsValid());
