...
public:
	...
	int Test3() {
		utils::AutoLock lock(g_lock);
		std::cout << "ThreadId: " << GetCurrentThreadId() << "\tCTest::Test3: " << 111111 << std::endl;
		return 111111;
	}
};
...
// 	task_schedule::PostTask(task_schedule::TaskThreadType::Pool, task_schedule::BindOnce(Test1, 11));
	...
	task_schedule::PostTaskAndReplyWithResult(task_schedule::TaskThreadType::UI, task_schedule::BindOnce(&CTest::Test3, a), task_schedule::BindOnce(Test1));
